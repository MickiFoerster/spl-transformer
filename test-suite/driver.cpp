...
#define ADJUST_FP_OPS(overall_fp_ops)  \
			ratio_real_to_measuread_FP_ops=1.; /* Do not scale! */  \
...
{ 
  if(stack_sizes_overview!=NULL) {
	  /* Table format is     THREAD_ID :  STACKc size  |   STACKi size   | STACKf size    */
...
int main(int argc, char* argv[])
{
	...
	while( ifs.good() ) {
		...
		if( line.find(searchpattern)!=string::npos )  {
			...
			n = oneMB_double;
			// As long as the memory needed for the data is bigger than 15% of free memory, reduce n:
			while( (n*n+n)*2*sizeof(double)/1024 > (mem_free_kb*15/100) ) 
				n--;

			//n=1000;
			if( mem_free_kb/(1024*1024) > 48 ) {
				// As long as matrix A and vector x does not take 256gb increase n:
	...
	// finite differences test ( OBSOLETE? Because this is done in external source file ? )
	...
	#endif
	// Time measurement should be placed into both versions.
	...
	for(int num_threads=1;num_threads<=omp_get_num_procs();num_threads*=2) {
		...
		cerr << "Adjoint scale test\n";
			// Adjust table for memory statistics 
