...
using namespace std;
  ...
  const int STACK_SIZE = (64*1024*1024); // This results in allocation of 1 gigabyte per thread!
#else
  //const int STACK_SIZE = 1024*1024;     // This results in allocation of to 16 megabyte for each thread!
...
class intStackBase {
	...
	public:
		...
		inline bool empty() { return (stack_counter==0); };
		inline void init() { 
			//stack_size=STACK_SIZE/omp_get_num_threads(); 
			stack_size=STACK_SIZE; 
